...
var dataURIs = [
  "https://6900bb26119175151fb10f1c4a56ed77862083b3-www.googledrive.com/host/0B7um4d7u6gG0U0VTUmJNb0NyUUE/Dsrip/AppForData%20/maggie.json"
...
d3.json(dataURIs[0], function(error, json) {
  ...
  json.children=json.children.map(function(c){
    ...
    if(c.name.match('Light-for-dates')){
      c.name='Light-for-dates'
    }
    console.log(c.name,c.children.length)
    return c
  })

  // remove what we don't like
  json.children=json.children.filter(function(c){
    return (!c.name.match('Light-for-dates'))
  })
  json.children=json.children.filter(function(c){
    return (!c.name.match('no name'))
  })
  // fix name case

  /*
  json.children=json.children.map(function(c){
    c.name=c.name.slice(0,1).toUpperCase()+c.name.slice(1)
    console.log(c.name)
    return c
  })
  */
